...
import { CheckIcon, ChevronIcon, MinusIcon } from "@/registry/lilt/ui/icons";
...
const isBranchElement = (element: HTMLElement) =>
...
const resolveDropPosition = (
  ...
  const band = Math.max(
    REORDER_BAND_MIN,
    Math.min(box.height * 0.25, REORDER_BAND_MAX)
  );
  if (offset < band) {
    return "before";
  }
...
> {
   ...
   * with `moveNode` from `@/registry/lilt/lib/tree`.
   */
...
export const Tree = ({
  ...
  const toggleSelected = useCallback(
    (value: string, event?: MouseEvent | KeyboardEvent) => {
      ...
      if (checkboxSelection) {
        ...
        const isChecked =
          getCheckState(childrenOf, selectedSet, value) === "checked";
        commitSelected(
  ...
  const moveRelative = useCallback(
    (element: HTMLElement, key: string) => {
      ...
      announce(
        `${labelOf(rootRef.current, value)} is already at the top level.`
      );
    },
  ...
  const activate = useCallback(
    (event: KeyboardEvent, current: HTMLElement, items: HTMLElement[]) => {
      ...
      if (event.key === "*") {
        event.preventDefault();
        const branches = getSiblingItems(current)
          .filter(isBranchElement)
          .map(getValueOf);
  ...
  return (
    ...
    >
      ...
      {icon ? (
        <span
          aria-hidden="true"
          className="flex shrink-0 text-[var(--lilt-text-subtle)]"
        >
          {icon}
        </span>
      ) : null}
